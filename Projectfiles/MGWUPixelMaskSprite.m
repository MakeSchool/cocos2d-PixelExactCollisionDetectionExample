...
@implementation MGWUPixelMaskSprite
...
-(id) initWithFile:(NSString *)filename alphaThreshold:(UInt8)alphaThreshold
{
	if ((self = [super initWithFile:filename]))
	{
		...
		{
			PixelMaskSpriteClass = [MGWUPixelMaskSprite class];
		}
...
{
	return [[self alloc] initWithFile:filename alphaThreshold:255];
}

+(id) spriteWithFile:(NSString *)filename alphaThreshold:(UInt8)alphaThreshold
{
	return [[self alloc] initWithFile:filename alphaThreshold:alphaThreshold];
}
...
-(BOOL) pixelMaskIntersectsPixelMaskSprite:(MGWUPixelMaskSprite*)other collisionPointBodyA:(CGPoint *)pointA collisionPointBodyB:(CGPoint *)pointB
{
...
#endif
                        {
                            CGPoint collisionPointBodyA = ccp(x, y);
                            collisionPointBodyA = CC_POINT_PIXELS_TO_POINTS(collisionPointBodyA);
                            *pointA = collisionPointBodyA;
                            
                            CGPoint collisionPointBodyB = ccp(otherX, otherY);
                            collisionPointBodyB= CC_POINT_PIXELS_TO_POINTS(collisionPointBodyB);
                            *pointB = collisionPointBodyB;
                            return YES;
                        }
                }
		}
...
-(BOOL) pixelMaskIntersectsRegularNode:(CCNode*)other collisionPointBodyA:(CGPoint *)pointA collisionPointBodyB:(CGPoint *)pointB
{
...
-(BOOL) pixelMaskIntersectsNode:(CCNode *)other collisionPointBodyA:(CGPoint *)pointA collisionPointBodyB:(CGPoint *)pointB {
	...
	if ([self intersectsNode:other])
	{
		...
		{
			MGWUPixelMaskSprite* maskSprite = (MGWUPixelMaskSprite*)other;
			return [self pixelMaskIntersectsPixelMaskSprite:maskSprite collisionPointBodyA:pointA collisionPointBodyB:pointB];
		}
		else
		{
			return [self pixelMaskIntersectsRegularNode:other collisionPointBodyA:pointA collisionPointBodyB:pointB];
		}
	}
...
{
    return [self pixelMaskIntersectsNode:other collisionPointBodyA:nil collisionPointBodyB:nil];
}
